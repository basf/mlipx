...
def drop_atom(current_pos, atoms, R):
    while True:
        distance_matrix = get_distances(current_pos, atoms.positions, cell=atoms.cell, pbc=True)[1]
...
def grid_placement(atoms, atom='C', grid_step=0.4):
    ...
    R       = np.array([covalent_radii[atomic_numbers[symbol]] + R_atom for symbol in _atoms.symbols])

    pos = []

    for i in x:
        for j in y:
            current_pos = np.array([i+j*(cell[1,0]/cell[1,1]),j+i*(cell[0,1]/cell[0,0]), height])
...
def gen_images(atoms, pos, mol='C'):
    ...
    for xyz in pos:
        _atoms = atoms.copy()
        if mol == 'CO':
            _atoms += Atoms(symbols=['C','O'], 
                    positions = [xyz, xyz+np.array([0,0,1.2])])

        elif mol == 'COH':
            _atoms += Atoms(symbols=['C','O','H'], 
                    positions = [xyz, xyz+np.array([0,0,1.2]), xyz+np.array([0,0.8,2.0])])

        elif mol == 'OH':
            _atoms += Atoms(symbols=['O','H'], 
                    positions = [xyz, xyz+np.array([0,0.8,0.8])])
...
def combine_constraints(atoms, index_from_last=0, nats_mol=1, freeze_ratio=0.5):
    idx = np.argsort(atoms.positions[:,2])[:int((len(atoms)-nats_mol)*freeze_ratio)]
...
def calculate(traj, generic_calculator, node_path, 
    ...
    for image in tqdm(traj, desc='calculation progress'):
        _image = image.copy()
        _image = combine_constraints(_image, index_from_last=index_from_last,
                nats_mol=nats_mol, freeze_ratio=freeze_ratio)
        calc = generic_calculator.get_calculator()             
        _image.calc = calc

        dyn = BFGS(_image, trajectory=f'{node_path}/current_relax.traj', 
...
def property_implemented(atoms, generic_calculator):
     ...
     if calc and not calc.calculation_required(atoms, ['energy']) and not calc.calculation_required(atoms, ['forces']):
...
def individual_run(traj, model, generic_calculator, node_path, basename, mol,
                   freeze_ratio=0.5, index_from_last=0, nats_mol=1, el=''):
    ...
    if os.path.exists(f'{node_path}/{mol}_{basename}_{model}_{el}.traj'):
        ...
        results = calculate(traj, generic_calculator, node_path,
            index_from_last=index_from_last, nats_mol=nats_mol, freeze_ratio=freeze_ratio)
...
def heatmap(model, Z, x1, x2, el, xrange, yrange):
    ...
    heatmap = ax.imshow(Z, extent=(xrange[0], xrange[1], yrange[0], yrange[1]),
    ...
    for i, label in enumerate(el):
        ...
        text = ax.annotate(label, (x1[i], x2[i]), textcoords="offset points",
                xytext=(2.5*sign, y_offset), ha='center',
...
class COSplitting(zntrack.Node):
    ...
    def run(self):  # noqa C901
        ...
        reference_data ={'Rh':[1.41, 0.21], 'Cu':[3.53, 1.02], 'Co':[1.70, -0.11], 
                'Fe':[1.28, -0.8], 'Ni':[1.53, 0.17], 'Pt':[2.12, 1.30], 
                'Ru':[1.26, -0.08], 'Ir':[1.60, -0.08], 'Pd':[1.53, 1.52],
                'Au':[4.75, 2.63],'Ag':[5.05, 1.90]} 
        ...
        mols[model]['CH4'] = pre_optimize(gas_phase('CH4', vacuum=self.vacuum),
                            self.model.get_calculator(), 
                            self.node_path / f'{model}_CH4_gas', 
                            cache_relaxations_path / f'{model}_CH4_gas', 
                            freeze_ratio=0).get_potential_energy()

        mols[model]['H2O'] = pre_optimize(gas_phase('H2O', vacuum=self.vacuum),
                            self.model.get_calculator(), 
                            self.node_path / f'{model}_H2O_gas',
                            cache_relaxations_path / f'{model}_H2O_gas',
                            freeze_ratio=0).get_potential_energy()

        mols[model]['H2'] = pre_optimize(gas_phase('H2', vacuum=self.vacuum),
                            self.model.get_calculator(), 
        ...
        for current_frame, bulk in tqdm(enumerate(self.data)):
            ...
            surf = gen_surf(bulk, miller=self.miller, layers=self.layers, 
                    vacuum=self.vacuum, supercell=self.supercell)

            calc = self.model.get_calculator()
            surf = pre_optimize(surf, calc, 
        ...
        for formula, surf in surfs[model].items():
            ...
            results     = individual_run(co_traj, model, self.model, self.node_path, '',
                    freeze_ratio=self.freeze_ratio, mol='CO', 
                    index_from_last=1, nats_mol=2, el=formula)
            ...
            _co_bound_surf  = c_bound_surf + Atom(symbol='O', 
                                    position=c_bound_surf[-1].position + (0,0,1.2))
            
            co_bound_surf  = pre_optimize(_co_bound_surf, 
                    self.model.get_calculator(), 
            ...
            co_bound_surf  = _c_temp_2 + Atom(symbol='O', position=_c_temp_2[-1].position + (0,0,1.2))
            pos, _atoms    = grid_placement(co_bound_surf, atom='H', grid_step=self.grid_step)
            h_traj         = gen_images(co_bound_surf, pos, mol='H')
            results        = individual_run(h_traj, model, self.model, self.node_path, '',
                                        freeze_ratio =self.freeze_ratio, mol='H', 
                                        index_from_last=0, nats_mol=3, el=formula)
            ...
            co_h_bound_surf  = pre_optimize(_co_h_bound_surf, 
                    self.model.get_calculator(), 
            ...
            pos, _atoms   = grid_placement(c_bound_surf, atom='O', grid_step=self.grid_step)
            oh_traj       = gen_images(c_bound_surf, pos, mol='OH')
            results       = individual_run(oh_traj, model, self.model, self.node_path, '', 
                                freeze_ratio=self.freeze_ratio, mol='OH', 
                                index_from_last=1, nats_mol=3, el=formula)

            c_oh_energies = []
            for _image in results:
                distance = get_distances(_image[-3].position, _image[-2].position, cell=_image.cell, pbc=True)[1][0][0]
                if distance >2:
            ...
            _c_oh_bound_surf  = _c_o_temp_2 + Atom(symbol='H', position=_c_o_temp_2[-1].position + (0,0.8,0.8))

            c_oh_bound_surf  = pre_optimize(_c_oh_bound_surf, self.model.get_calculator(), 
            ...
            distance = get_distances(c_oh_bound_surf[-3].position, c_oh_bound_surf[-2].position, cell=c_oh_bound_surf.cell, pbc=True)[1][0][0]
            if distance >2:
                bond_broken=True
            else:
                bond_broken=False
            ...
            results     = individual_run(c_traj, model, self.model, self.node_path, '',
                    freeze_ratio=self.freeze_ratio, mol='C', 
                    index_from_last=0, nats_mol=1, el=formula)


            _c_bound_surf = results[np.argmin([_image.get_potential_energy() for _image in results])]
            _c_bound_surf  = relocate_atom(_c_bound_surf, idx = len(_c_bound_surf)-1)
            c_bound_surf  = pre_optimize(_c_bound_surf, self.model.get_calculator(), 
                    self.node_path / f'{model}_{formula}_C_final',
                    cache_relaxations_path / f'{model}_{formula}_C_final',
                    nats_mol=1, freeze_ratio=self.freeze_ratio)
            ...
            results     = individual_run(o_traj, model, self.model, self.node_path, '',
                    freeze_ratio=self.freeze_ratio, mol='O', 
                    index_from_last=0, nats_mol=1,el=formula)

            _o_bound_surf = results[np.argmin([_image.get_potential_energy() for _image in results])]
            _o_bound_surf = relocate_atom(_o_bound_surf, idx = len(_o_bound_surf)-1)
            o_bound_surf  = pre_optimize(_o_bound_surf, self.model.get_calculator(), 
            ...
            model_results[model][formula]['barrier(from bep rpbe)'] =  float(barrier_bep(c_oh_en - co_h_en))
            model_results[model][formula]['CH4->C*+2H2'] = float(c_en - (surf.get_potential_energy() + mols[model]['CH4'] - 2*mols[model]['H2']))
            model_results[model][formula]['H2O->O*+H2'] = float(o_en - (surf.get_potential_energy() + mols[model]['H2O'] - mols[model]['H2']))
            model_results[model][formula]['bond_broken'] = bond_broken

            x1.append(c_en - (surf.get_potential_energy() + mols[model]['CH4'] - 2*mols[model]['H2']))
            x2.append(o_en - (surf.get_potential_energy() + mols[model]['H2O'] - mols[model]['H2']))
        ...
        model_results[model]['interpolation'] = {'formula':f"co_h->c_oh = {m1:.2f} * C + {m2:.2f} * O + {b:.2f}",
                                                 'm1':m1,
                                                 'm2':m2,
                                                 'b':b,
                                                 'r2':r_squared,
    ...
    def figures(self) -> dict[str, go.Figure]:
        ...
        fig.update_layout(
            title=f"Reaction Energy for H-assisted CO Splitting",
            xaxis_title="Formula",
            yaxis_title="dE (eV)",
        )
